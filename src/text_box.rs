...
impl TextBox {
    ...
    pub fn set_contents(&mut self, text: Vec<char>, text_renderer: &TextRenderer) {
        self.reset();
    ...
    fn handle_clipboard(&mut self, keyboard: &mut Keyboard, clipboard: &mut Clipboard) {
        if keyboard.pressed_with_mods(Key::V, CTRL) {
            ...
            if let Ok(mut text) = clipboard.get_text() {
                ...
                let length_limit = self.length_limit - self.buffer.len();
    ...
    pub fn draw_styled(
        ...
        //renderer.draw_rect_xy_wh((self.x, self.y - 5), (self.width, 30), 0xFF_00_00_FF);
        renderer.text().apply_scissors((self.x, window_height - 30 - self.y + 5), (self.width as _, 30));
        ...
        if !sel.is_empty() {
            ...
            let x = (self.x + sel_start_x).saturating_sub(self.visible_start).clamp(self.x, self.x + self.width);
        ...
        if sel.is_empty() && self.active && (time - self.last_keypress) % 1.0 < 0.5 {
            const SCALE: u16 = 3;
            renderer.draw_rect_xy_wh(
                ((self.x + cursor_x - (SCALE - 1)).saturating_sub(self.visible_start), y - 2 * SCALE),
                (2, 10 * SCALE),
                style.cursor_color,
            );
        }
...
/* pub fn draw_styled(
    ...
    renderer.draw_rect_xy_wh((self.x + self.visible_start, self.y - 5), (self.width, 30), 0xFF_00_00_FF);
    //renderer.text().apply_scissors((self.visible_start, self.y), (self.width as _, 30));
    ...
    if !sel.is_empty() {
        ...
        const SCALE: u16 = 3;
        renderer.draw_rect_xy_wh(
            ((x + sel_start_x), y - 2 * SCALE),
            (sel_width.min(self.width), 10 * SCALE),
            0xA0_C7_F2_FF,
        );
    }
    ...
    if self.active && (time - self.last_keypress) % 1.0 < 0.5 {
        const SCALE: u16 = 3;
        renderer.draw_rect_xy_wh(
            ((x + cursor_x - (SCALE - 1)).saturating_sub(self.visible_start).max(self.x), y - 2 * SCALE),
            (2, 10 * SCALE),
            style.cursor_color,
        );
    }
